    ...
    for (size_t deviceId = 0; deviceId < ASCEND_NPU_CORE_NUM; deviceId++) {
      ...
      std::thread worker(
        [&, deviceId] {
          ...
          void* inputMem;
          ret = aclrtMalloc(&inputMem, 2 * ASCEND_NPU_BATCH_SIZE * kInputPlanes * 8 * 8, ACL_MEM_MALLOC_HUGE_FIRST);
          if (ret != ACL_SUCCESS) {
          ...
          auto inputBuffer = aclCreateDataBuffer(inputMem, 2 * ASCEND_NPU_BATCH_SIZE * kInputPlanes * 8 * 8);
          ...
          ret = aclrtMallocHost(&policyOutputMemHost, 2 * ASCEND_NPU_BATCH_SIZE * 1858);
          if (ret != ACL_SUCCESS) {
            aclmdlDestroyDataset(input);
            aclDestroyDataBuffer(inputBuffer);
            aclrtFree(inputMem);
          ...
          for (;;) {
            ...
            ret = aclrtMemcpy(inputMem, 2 * ASCEND_NPU_BATCH_SIZE * kInputPlanes * 8 * 8, computeTask.inputData, 2 * ASCEND_NPU_BATCH_SIZE * kInputPlanes * 8 * 8, ACL_MEMCPY_HOST_TO_DEVICE);
            if (ret != ACL_SUCCESS) {
              aclrtFreeHost(computeTask.inputData);
              AscendNPU::isOk_ = false;
              computeTask.computation->result_queue_.push({});
            ...
            for (size_t i = 0; i < aclmdlGetDatasetNumBuffers(output); i++) {
              ...
              switch (aclGetDataBufferSizeV2(buffer)) {
                case 2 * ASCEND_NPU_BATCH_SIZE * 1858:
                  ret = aclrtMemcpy(policyOutputMemHost, 2 * ASCEND_NPU_BATCH_SIZE * 1858, aclGetDataBufferAddr(buffer), 2 * computeTask.batchSize * 1858, ACL_MEMCPY_DEVICE_TO_HOST);
                  if (ret != ACL_SUCCESS) {
            ...
            for (size_t i = 0; i < computeTask.batchSize * 1858; i++) {
...
void AscendNPUComputation::ComputeBlocking() {
  ...
  while (batchOffset != totalBatchSize) {
    ...
    for (size_t i = batchOffset; i < batchOffset + batchBatchSize; i++) { // 16
      for (const auto& plane : raw_input_[i]) { // 112
        aclFloat16 value = aclFloatToFloat16(plane.value);
        for (auto bit : IterateBits(plane.mask)) { // 8 * 8
          *(iter + bit) = value;
        }
        iter += 64;
      }
    ...
    if (batchOffset == 0) {
      ...
      for (size_t i = 0; i < 8; i++) {
        for (size_t j = 0; j < 8; j++) {
          s << aclFloat16ToFloat(static_cast<aclFloat16*>(inputData)[(i * 8) + j]) << " ";
        }
        s << std::endl;
      }
      AscendNPU::Log(s.str());
  ...
  for (size_t i = 0; i < 8; i++) {
    for (size_t j = 0; j < 8; j++) {
      s << aclFloat16ToFloat(policy_head_output_.data()[(i * 8) + j]) << " ";
    }
    s << std::endl;
  }
  AscendNPU::Log(s.str());
...
float AscendNPUComputation::GetPVal(int sample, int move_id) const {
  return aclFloat16ToFloat(policy_head_output_[sample * 1858 + move_id]);
}
...
std::unique_ptr<Network> MakeAscendNPUNetwork(
  ...
  if (system(("atc --model \"" + cacheFile + ".onnx\" --framework 5 --input_shape \"/input/planes:" + std::to_string(ASCEND_NPU_BATCH_SIZE) + "," + std::to_string(kInputPlanes) + ",8,8\" --input_format=NCHW --output \"" + cacheFile + "\" --soc_version Ascend310").c_str()) != 0) {
...
#else
  return std::make_unique<AscendNPUNetwork>(onnxModel, "./.ascend-npu-cache-lc/0.om");
#endif
}
